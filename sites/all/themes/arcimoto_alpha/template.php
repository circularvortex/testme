...
function phptemplate_preprocess_page($vars) {
  ...
  // Set up page templates based on URL alias.  Requires pathauto.
...
/** 
*   Add variables to the Ubercart invoice
**/


function phptemplate_preprocess_uc_order($variables) {

	global $user;

	if (in_array('angel-bronze', array_values($user->roles))) {
	    $wingstype = 'bronze';
	}
	elseif (in_array('angel-silver', array_values($user->roles))) {
	    $wingstype = 'silver';
	}
	elseif (in_array('angel-gold', array_values($user->roles))) {
	    $wingstype = 'gold';
	}
	elseif (in_array('angel-mithril', array_values($user->roles))) {
	    $wingstype = 'mithril';
	}
	elseif (in_array('angel-unobtainium', array_values($user->roles))) {
	    $wingstype = 'unobtainium';
	};
	
	$variables['wingstype'] = $wingstype;
}
...
/* TODO This seems to fuck up the field displays on Ubercart checkout page and user/#/edit pages.  Is it even necessary? */
...
/*  Counts number of preorders for display (deprecated immediately)
